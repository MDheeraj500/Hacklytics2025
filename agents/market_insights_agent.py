...
"""
        ...
        if data_path:
            ...
            self.models = [col.replace('_Sentiment', '')
                           for col in self.df.columns if col.endswith('_Sentiment')]
        else:
            self.df = None
            self.models = []
    ...
    def analyze_price_sentiment_relationship(self, model_name: str) -> dict:
        ...
        sentiment_col = f"{model_name}_Sentiment"
        sentiment_lead = self.df[sentiment_col].shift(-1)
        price_lead = self.df[price_col].shift(-1)
        return {
...
"""
        try:
            ...
            print(f"[MarketInsightsAgent] Error in get_market_summary with query: {query}\nError: {e}")
            raise
